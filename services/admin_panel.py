...
class AdminPanel:
    ...
    def get_welcome_message(self) -> str:
        ...
        Якщо в сховищі немає явного welcome_message, підтягуємо templates['start_message']
        щоб синхронізуватися з фактичним повідомленням /start.
        """
        stored = self.data.get("welcome_message")
        if isinstance(stored, str) and stored.strip():
            return stored
        # Fallback to message_templates.json -> start_message
        try:
            from config.message_templates import load_messages
            msgs = load_messages()
            tmpl = (msgs or {}).get("start_message")
        ...
        return (
            "Вітаю! Я — Glock FX BOT, ваш надійний помічник у світі трейдингу.\n\n"
